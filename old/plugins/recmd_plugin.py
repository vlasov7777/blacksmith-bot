							...
							if new_cmd not in COMMANDS and new_cmd not in RECMDS['name']:
								if command not in RECMDS['list']:
									if len(new_cmd) <= 12:
										...
										syntax = COMMANDS[command]['syntax'].decode('utf-8')
										syntax = syntax.replace(command, new_cmd)
										COMMANDS[command]['syntax'] = syntax.encode('utf-8')
										examples = []
										for example in COMMANDS[command]['examples']:
											example = example.decode('utf-8')
											example = example.replace(command, new_cmd)
											examples.append(example.encode('utf-8'))
										COMMANDS[command]['examples'] = examples
										if len(list) >= 4:
											x = list[3].strip()
											if x.count('!'):
												x = x.replace('!', '')
...
register_command_handler(handler_reinit_command, 'заменить', ['суперадмин','все'], 100,'Меняет доступ или имя (или имя и доступ сразу) указанной команды\nБез параметров покажет список изменённых команд.', 'заменить [команда] [доступ/acc/имя/name] [доступ/имя (!доступ)]', ['заменить пинг имя понг','заменить пинг внорму','заменить тест доступ 20','заменить','заменить бан имя втопку !15'])
