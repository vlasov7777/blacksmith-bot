...
register_command_handler(handler_commoff, 'отключить', ['админ','все'], 20, 'Отключает определённые команды для текущей конфы, без параметров показывает список уже отключенных команд.', 'отключить [команды]', ['отключить','отключить тык зацени версия пинг'])
register_command_handler(handler_common, 'включить', ['админ','все'], 20, 'Включает определённые команды для текущей конфы.', 'включить [команды]', ['включить тык зацени версия пинг'])
