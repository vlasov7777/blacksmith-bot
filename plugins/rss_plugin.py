	...
	if name:
		if name in RSS_CACHE.keys():
			try:
				...
				info = u'Это супер-дупер мегаканал с кучей полезной херни!'
...
register_command_handler(handler_rss_control, 'трансляция', ['трансляции','все'], 30, 'Начать/закончить трансляцию.', 'трансляция [старт/start/стоп/stop]', ['трансляция'])
register_command_handler(handler_rss_channels, 'канал', ['трансляции','все'], 100, 'Добавляет/Удаляет RSS канал.', 'канал [+/-] [name] [url]', ['канал + slashdot http://www.slashdot.org/slashdot.rdf','канал - slashdot'])
register_command_handler(handler_rss_subscribe, 'подписка', ['трансляции','все'], 30, 'Добавляет/Удаляет подписку.', 'подписка [+/-] [name]', ['подписка + slashdot','подписка - slashdot'])
register_command_handler(handler_rss_info, 'каналы', ['трансляции','все'], 30, 'Даёт инфу о канале или список каналов.', 'каналы [name]', ['каналы slashdot', 'каналы'])
