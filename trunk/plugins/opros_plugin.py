...
register_command_handler(handler_opros, 'опрос', ['суперадмин','все'], 100, 'Глобальный опрос. Бот отсылает сообщение по всем конфам где сидит (сначала в чат(1 раз на комнату), а далее через каждые 20 минут отсылает опрос модераторам в приваты (тем что ещё не видели опроса и тем что ещё не не ответил и не получал опрос в приват)), без параметра покажет текужее состояние опроса\nBy WitcherGeralt', 'опрос <стоп/старт>', ['опрос старт','опрос стоп','опрос'])
register_command_handler(handler_opros_base, 'пункты', ['суперадмин','все'], 100, 'Заполнить опрос, без параметров покажет текущее боди опроса\nBy WitcherGeralt', 'пункты <заголовок/пункт> <текст>', ['пункты заголовок Самый крутой ФК в Российской премьер лиге','пункты 1 ЛОКО'])
register_command_handler(handler_opros_otvet, 'вариант', ['разное','все'], 15, 'Ответ на глобальный опрос номером пункта\nBy WitcherGeralt', 'вариант <номер варианта>', ['вариант 1'])
register_command_handler(handler_opros_mind, 'вариант*', ['разное','все'], 15, 'Ответ фразой на глобальный опрос\nBy WitcherGeralt', 'вариант* <фраза>', ['вариант* да ну это всё! ни с одним пунктом не согласен'])
