...
#  Modifications :
#		© 2010 WitcherGeralt [WitcherGeralt@rocketmail.com]
#		© 2011 simpleApps (http://simpleapps.ru)

turn_msgs = {}

def getTable():
	return dict(zip(u"qwertyuiop[]asdfghjkl;'zxcvbnm,.Ю`йцукенгшщзхъфывапролджэячсмитьбю.ёQWERTYUIOP{}ASDFGHJKL:\"ZXCVBNM<>Б~ЙЦУКЕНГШЩЗХЪФЫВАПРОЛДЖЭЯЧСМИТЬБЮ,Ё", u"йцукенгшщзхъфывапролджэячсмитьбю.ёqwertyuiop[]asdfghjkl;'zxcvbnm,.ю`ЙЦУКЕНГШЩЗХЪФЫВАПРОЛДЖЭЯЧСМИТЬБЮ,ЁQWERTYUIOP{}ASDFGHJKL:\"ZXCVBNM<>б~"))
#'	
		...
		if not body:
			...
			if body != None:
				if body not in (u"турн", "turn"):
					...
					rebody = reduce(lambda x,y: getTable().get(x,x)+getTable().get(y,y), body)
					for x in list:
			...
			reply(type, source, u'Держи: '+reduce(lambda x,y: getTable().get(x,x)+getTable().get(y,y), body))
	else:
		reply(type, source, u'команда выполняется только в чате')

def handler_turn_save_msg(raw, type, source, body):
	if (type == 'public', source[1] in turn_msgs, source[2]):
...
command_handler(handler_turn_last, 10, "turn")
